...
function module:Init()
	...
	if Model then
			...
			local function RagdollV3()
				...
				local function scan(ch)
					for i = 1, #ch do
						...
						if (ch[i]:IsA("ForceField") or ch[i].Name == "HumanoidRootPart") or ((ch[i]:IsA("Motor6D")) and ch[i].Name ~= "HeadWeld" and ch[i].Name ~= "AttachementWeld") then
							ch[i]:destroy()
						end
					end
				end
				...
				local function scanc(ch)
					for _, obj in pairs(ch:GetChildren()) do
						scanc(obj)
						if obj:IsA("Script") or obj:IsA("LocalScript") or ((obj:IsA("Motor6D")) and obj.Name ~= "AttachementWeld") or obj:IsA("ForceField") or (obj:IsA("Snap") and obj.Parent.Name == "Torso")
				...
				game:GetService("Debris"):AddItem(script, cleanUpTime)
				fhum.MaxHealth = 100
		...
		warn("Not a valid NPC (missing model)")
	end
end
